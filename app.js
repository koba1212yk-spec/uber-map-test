...
function setGeoMode(mode) {
    ...
    if (iconTarget && iconCompass) {
        if (mode === 'A') {
            ...
            iconCompass.setAttribute('fill', '#5F6368'); 
            if (cone) cone.classList.remove('show');
            stopCompass(); 
        } else if (mode === 'C') {
            iconTarget.style.display = 'none';
            iconCompass.style.display = 'block';
            iconCompass.setAttribute('fill', '#1A73E8'); 
...
function jumpToCurrentLocation() {
    navigator.geolocation.getCurrentPosition(pos => {
        ...
        // 天気を「現在地（isCustom = false）」として取得し直す
        fetchWeatherData(pos.coords.latitude, pos.coords.longitude, false);
...
map.on('load', () => {
    ...
    map.addLayer({ 
        ...
        paint: { 
            ...
            'circle-color': [
                'case', ['get', 'isSelected'],
                [ 'match', ['get', 'category'], '🏢 建物・入口', '#1A73E8', '🅿️ 駐輪スポット', '#34A853', '⚠️ 注意・取締り', '#EA4335', '🚻 トイレ・公園', '#FBBC04', '#9AA0A6' ],
                '#FFFFFF'
            ], 
            'circle-radius': 16, 
            'circle-stroke-width': 3, 
            'circle-stroke-color': [ 'match', ['get', 'category'], '🏢 建物・入口', '#1A73E8', '🅿️ 駐輪スポット', '#34A853', '⚠️ 注意・取締り', '#EA4335', '🚻 トイレ・公園', '#FBBC04', '#9AA0A6' ]
        } 
    });

    map.addLayer({ 
        id: 'unclustered-icon', type: 'symbol', source: 'memos', filter: ['!', ['has', 'point_count']], 
        layout: { 'icon-image': ['get', 'iconName'], 'icon-size': 0.7, 'icon-allow-overlap': true },
        paint: {
            'icon-opacity': ['get', 'opacity'], 
            'icon-color': [ 'case', ['get', 'isSelected'], '#FFFFFF', [ 'match', ['get', 'category'], '🏢 建物・入口', '#1A73E8', '🅿️ 駐輪スポット', '#34A853', '⚠️ 注意・取締り', '#EA4335', '🚻 トイレ・公園', '#FBBC04', '#9AA0A6' ] ]
        }
    });
...
if (geoBackBtn) {
    geoBackBtn.addEventListener('click', () => {
        if (geoMode === 'A') { jumpToCurrentLocation(); } else if (geoMode === 'B') { map.flyTo({ zoom: 20, duration: 800 }); setGeoMode('C'); } else if (geoMode === 'C') { setGeoMode('B'); }
    });
}
...
if (btnQuickMemo) {
    btnQuickMemo.addEventListener('click', () => {
        if (currentLocationMarker) {
            const ll = currentLocationMarker.getLngLat();
            const coords = [ll.lng, ll.lat];
            map.flyTo({ center: coords, zoom: 20, duration: 800 });
            updateCurrentLocation(coords);
            setTimeout(() => { map.fire('contextmenu', { lngLat: { lng: coords[0], lat: coords[1] } }); }, 850);
...
setInterval(() => {
    ...
    if (isTickerFlipped) {
        content.classList.remove('flipped'); tickerIndex = (tickerIndex + 1) % latestMemos.length; 
    } else {
...
async function loadProfile() {
    try {
        ...
        if (snap.exists()) {
            ...
            document.getElementById('profileName').value = myProfileName;
            if (isNameLocked) {
                document.getElementById('nameChangeWarning').style.display = 'none'; document.getElementById('profileName').disabled = true;
            }
...
document.getElementById('submitOpinionBtn').addEventListener('click', async () => { 
    ...
    try { await addDoc(collection(db, "opinions"), { text: text, senderId: currentUserId, createdAt: new Date() }); alert('送信完了！ご協力ありがとうございます。'); document.getElementById('opinionInput').value = ""; } catch (e) {} 
});
...
function openMemoAddSheet(lngLat) {
    ...
    document.getElementById('memoBottomSheet').classList.remove('show'); document.getElementById('filterBottomSheet').classList.remove('show');
    const actionPanel = document.getElementById('memoActionPanel'); actionPanel.classList.remove('peek'); actionPanel.classList.add('show');
}

const closeMemoForm = () => { 
    const actionPanel = document.getElementById('memoActionPanel'); actionPanel.classList.remove('show'); actionPanel.classList.remove('peek'); 
    document.getElementById('btnRemoveImage').click(); document.getElementById('memoTextInput').value = ""; editingMemoId = null; if (tempPinMarker) tempPinMarker.remove(); 
};
...
async function handleMemoImage(file, btnTextId, defaultText) {
    if (!file) return; 
    if (selectedImageDataUrls.length >= 3) { alert("画像は最大3枚までです"); return; }
    ...
    if(!countLabel) {
        countLabel = document.createElement('div'); countLabel.id = 'memoImageCountLabel'; countLabel.style.textAlign = 'center'; countLabel.style.fontSize = '12px'; countLabel.style.color = '#1A73E8'; countLabel.style.fontWeight = 'bold'; countLabel.style.marginTop = '4px';
    ...
    if (selectedImageDataUrls.length >= 3) { document.getElementById('memoCameraBtnText').parentNode.style.display = 'none'; document.getElementById('memoImageBtnText').parentNode.style.display = 'none'; }
...
document.getElementById('btnRemoveImage').addEventListener('click', () => { 
    selectedImageDataUrls = []; document.getElementById('memoImageInput').value = ""; document.getElementById('memoCameraInput').value = ""; document.getElementById('memoImagePreviewContainer').style.display = 'none'; document.getElementById('memoCameraBtnText').parentNode.style.display = 'inline-block'; document.getElementById('memoImageBtnText').parentNode.style.display = 'inline-block'; 
    const countLabel = document.getElementById('memoImageCountLabel'); if(countLabel) countLabel.textContent = "";
});
...
document.getElementById('btnSaveMemo').addEventListener('click', async () => {
    ...
    try {
        ...
        for (let i = 0; i < selectedImageDataUrls.length; i++) {
            ...
            if (dataUrl.startsWith('http')) { finalUrls.push(dataUrl); } else {
                const storageRef = ref(storage, `memos/${Date.now()}_${currentUserId}_${i}.jpg`); 
                await uploadString(storageRef, dataUrl, 'data_url'); finalUrls.push(await getDownloadURL(storageRef)); 
            }
        }
        ...
        const bounceEl = document.createElement('div'); bounceEl.className = 'success-bounce-pin';
        let pinColor = '#06C167'; 
        if (category === '🏢 建物・入口') pinColor = '#1A73E8'; else if (category === '🅿️ 駐輪スポット') pinColor = '#34A853'; else if (category === '⚠️ 注意・取締り') pinColor = '#EA4335'; else if (category === '🚻 トイレ・公園') pinColor = '#FBBC04';
        bounceEl.style.backgroundColor = pinColor;

        const bounceMarker = new maplibregl.Marker({ element: bounceEl, anchor: 'center' }).setLngLat(targetLngLat).addTo(map);
        setTimeout(() => { bounceMarker.remove(); }, 1500);
...
function applyFilters() {
    ...
    allMemosData.forEach(data => {
        ...
        if (!isSelected) {
            let overlapCount = 0;
            for (let c of usedCoords) { const dist = Math.sqrt(Math.pow(c.lng - displayLng, 2) + Math.pow(c.lat - displayLat, 2)); if (dist < 0.00005) overlapCount++; }
...
function openMemoBottomSheet(props) {
    ...
    if(images.length > 0) { 
        ...
        images.forEach((url, idx) => {
            const img = document.createElement('img'); img.src = url; img.onclick = () => openImageViewer(url); scrollArea.appendChild(img);
            const dot = document.createElement('div'); dot.className = 'dot' + (idx === 0 ? ' active' : ''); dotsArea.appendChild(dot);
        });
        if (images.length > 1) {
            counterLabel.style.display = 'block'; counterLabel.textContent = `1 / ${images.length}`; dotsArea.style.display = 'flex';
            scrollArea.onscroll = () => {
                const index = Math.round(scrollArea.scrollLeft / scrollArea.clientWidth); counterLabel.textContent = `${index + 1} / ${images.length}`;
                const dots = dotsArea.querySelectorAll('.dot'); dots.forEach((d, i) => { if (i === index) d.classList.add('active'); else d.classList.remove('active'); });
            };
        } else { counterLabel.style.display = 'none'; dotsArea.style.display = 'none'; }
...
document.getElementById('btnLikeMemo')?.addEventListener('click', async () => {
    ...
    try { await updateDoc(doc(db, "memos", currentOpenMemoId), { likesCount: increment(1) });
...
document.getElementById('btnReportMemo')?.addEventListener('click', async () => {
    ...
    try { await updateDoc(doc(db, "memos", currentOpenMemoId), { reportCount: increment(1) }); document.getElementById('sheetReportCount').textContent = parseInt(document.getElementById('sheetReportCount').textContent || 0) + 1; alert("通報しました。ご協力ありがとうございます。"); } catch(e) {}
});

document.getElementById('btnDeleteMemo')?.addEventListener('click', async () => {
    if(!currentOpenMemoId || !confirm("削除しますか？")) return;
    await deleteDoc(doc(db, "memos", currentOpenMemoId)); document.getElementById('memoBottomSheet').classList.remove('show');
    const navBar = document.getElementById('memoGroupNav'); if (navBar) navBar.style.display = 'none'; loadMemosToMap();
});
...
document.getElementById('btnEditMemo')?.addEventListener('click', () => {
    ...
    if (selectedImageDataUrls.length > 0) {
        document.getElementById('memoImagePreview').src = selectedImageDataUrls[0]; document.getElementById('memoImagePreviewContainer').style.display = 'block';
        let countLabel = document.getElementById('memoImageCountLabel');
        if(!countLabel) { countLabel = document.createElement('div'); countLabel.id = 'memoImageCountLabel'; countLabel.style.textAlign = 'center'; countLabel.style.fontSize = '12px'; countLabel.style.color = '#1A73E8'; countLabel.style.fontWeight = 'bold'; countLabel.style.marginTop = '4px'; document.getElementById('memoImagePreviewContainer').appendChild(countLabel); }
        countLabel.textContent = `${selectedImageDataUrls.length}枚選択中 (最大3枚)`;
        if (selectedImageDataUrls.length < 3) { document.getElementById('memoCameraBtnText').parentNode.style.display = 'inline-block'; document.getElementById('memoImageBtnText').parentNode.style.display = 'inline-block'; } else { document.getElementById('memoCameraBtnText').parentNode.style.display = 'none'; document.getElementById('memoImageBtnText').parentNode.style.display = 'none'; }
    } else { document.getElementById('btnRemoveImage').click(); }

    document.getElementById('memoBottomSheet').classList.remove('show'); document.getElementById('memoBottomSheet').classList.remove('peek');
    const actionPanel = document.getElementById('memoActionPanel'); actionPanel.classList.remove('peek'); actionPanel.classList.add('show');
...
async function openProfileModal(targetId, targetName) {
    document.getElementById('modalName').textContent = targetName; document.getElementById('modalArea').textContent = "読込中..."; document.getElementById('modalTime').textContent = "読込中..."; document.getElementById('modalLikesCount').textContent = "-"; document.getElementById('modalTags').innerHTML = "";
    document.getElementById('profileModalOverlay').style.display = 'block'; document.getElementById('profileModal').style.display = 'block';
    try { const snap = await getDoc(doc(db, "profiles", targetId)); if (snap.exists()) { const data = snap.data(); document.getElementById('modalArea').textContent = data.mainArea || "未設定"; document.getElementById('modalTime').textContent = data.mainTime || "未設定"; document.getElementById('modalLikesCount').textContent = data.totalLikes || 0; const tags = [...(data.vehicles || []), ...(data.services || [])]; if(tags.length) tags.forEach(t => { const span = document.createElement('span'); span.className='tag'; span.textContent=t; document.getElementById('modalTags').appendChild(span); }); } } catch(e) {}
}
...
memoSheet.addEventListener('touchend', (e) => {
    ...
    if (diff > 50 && memoSheet.classList.contains('show') && !memoSheet.classList.contains('peek')) { if (memoSheet.scrollTop === 0) memoSheet.classList.add('peek'); } 
    else if (diff < -50 && memoSheet.classList.contains('peek')) { memoSheet.classList.remove('peek'); }
});
...
async function fetchWeatherData(lat, lng, isCustom = false) {
    try {
        ...
        // 2. 地名の取得（指定エリアモードの時のみ）
        let locationName = "現在地周辺";
        if (isCustom) {
            try {
                const geoUrl = `https://api.maptiler.com/geocoding/${lng},${lat}.json?key=${MAPTILER_KEY}&language=ja`;
                const geoRes = await fetch(geoUrl);
                const geoData = await geoRes.json();
                if (geoData.features && geoData.features.length > 0) {
                    // 市区町村にあたる情報を優先的に探す
                    const matchFeature = geoData.features.find(f => f.place_type.includes('municipality') || f.place_type.includes('submunicipality') || f.text.match(/[市区町村]$/));
                    if (matchFeature) locationName = matchFeature.text;
                    else locationName = geoData.features[0].text; // 見つからなければ一番近い地名
                }
...
const btnSelectWeatherArea = document.getElementById('btnSelectWeatherArea');
const weatherTargetMark = document.getElementById('weatherTargetMark');
const btnWeatherConfirm = document.getElementById('btnWeatherConfirm');
...
// マップを動かしたら自動で天気シートを閉じる
function closeWeatherSheetOnMapMove() { 
    if (weatherSheet && weatherSheet.classList.contains('show')) { weatherSheet.classList.remove('show'); } 
}
map.on('dragstart', closeWeatherSheetOnMapMove); 
map.on('zoomstart', closeWeatherSheetOnMapMove);

// 「📍 別のエリアの天気を見る」ボタン
if (btnSelectWeatherArea) {
    btnSelectWeatherArea.addEventListener('click', () => {
        weatherSheet.classList.remove('show');
        weatherTargetMark.style.display = 'block'; // マップ中央のターゲットを表示
        btnWeatherConfirm.style.display = 'flex'; // 下部の決定ボタンを表示
    });
}

// 「📍 ここで決定」ボタン
if (btnWeatherConfirm) {
    btnWeatherConfirm.addEventListener('click', () => {
        weatherTargetMark.style.display = 'none';
        btnWeatherConfirm.style.display = 'none';
        
        // マップの中心座標を取得して天気を更新（isCustom = true）
