...
if (searchInput && searchSuggestions) {
    searchInput.addEventListener('input', (e) => {
        ...
        searchTimeout = setTimeout(async () => {
            searchSuggestions.innerHTML = ''; let hasResults = false;
            ...
            if (localMatches.length > 0) {
                ...
                groupLabel.textContent = '📝 みんなの攻略メモ'; searchSuggestions.appendChild(groupLabel);
                ...
                localMatches.forEach(m => {
                    ...
                    div.onclick = () => { map.flyTo({ center: [m.lng, m.lat], zoom: 18 }); searchSuggestions.style.display = 'none'; searchInput.blur(); openMemoBottomSheet(m); };
            ...
            try {
                ...
                if (data.length > 0) {
                    ...
                    if (localMatches.length > 0) {
                        ...
                        groupLabel.textContent = '📍 地図・住所'; searchSuggestions.appendChild(groupLabel);
                    }
                    data.forEach(item => {
                        const div = document.createElement('div'); div.className = 'suggestion-item';
                        ...
                        div.onclick = () => { map.flyTo({ center: [parseFloat(item.lon), parseFloat(item.lat)], zoom: 16 }); searchInput.value = item.name ? item.name : formattedAddress.split(' ')[0]; searchSuggestions.style.display = 'none'; searchInput.blur(); };
                        searchSuggestions.appendChild(div);
                    });
                }
    ...
    searchInput.addEventListener('keypress', async (e) => { 
        if(e.key === 'Enter') {
            ...
            try {
                ...
                if (localMatch) { map.flyTo({ center: [localMatch.lng, localMatch.lat], zoom: 18 }); searchSuggestions.style.display = 'none'; searchInput.blur(); openMemoBottomSheet(localMatch); return; }
                ...
                const data = await res.json();
                if(data.length > 0) { map.flyTo({ center: [parseFloat(data[0].lon), parseFloat(data[0].lat)], zoom: 16 }); searchSuggestions.style.display = 'none'; searchInput.blur(); } else { alert("見つかりませんでした"); }
            } catch (err) {}
        } 
    });
    document.addEventListener('click', (e) => { if (!searchInput.contains(e.target) && !searchSuggestions.contains(e.target)) { searchSuggestions.style.display = 'none'; } });
}
...
document.getElementById('tickerClickArea').addEventListener('click', () => { if (latestMemos.length === 0) return; const targetMemo = isTickerFlipped ? latestMemos[tickerIndex] : latestMemos[(tickerIndex === 0 ? latestMemos.length - 1 : tickerIndex - 1)]; map.flyTo({ center: [targetMemo.lng, targetMemo.lat], zoom: 16 }); openMemoBottomSheet(targetMemo); });
...
document.getElementById('saveProfileBtn').addEventListener('click', async () => { let finalName = document.getElementById('profileName').value.trim() || myProfileName; const area = document.getElementById('profileArea').value.trim(); const time = document.getElementById('profileTime').value.trim(); const vehicles = Array.from(document.querySelectorAll('input[name="vehicleTag"]:checked')).map(el => el.value); const services = Array.from(document.querySelectorAll('input[name="serviceTag"]:checked')).map(el => el.value); let lockFlag = isNameLocked; if (!isNameLocked && finalName !== myProfileName) lockFlag = true; try { await setDoc(doc(db, "profiles", currentUserId), { displayName: finalName, mainArea: area, mainTime: time, vehicles: vehicles, services: services, nameChanged: lockFlag, updatedAt: new Date() }, { merge: true }); localStorage.setItem('deliMapTempName', finalName); await loadProfile(); document.getElementById('profileEditMode').style.display = 'none'; document.getElementById('profileReadMode').style.display = 'block'; } catch (error) {} });
...
function openMemoAddSheet(lngLat, zoomIn = false) {
    ...
    if (lngLat) {
        ...
        document.getElementById('memoTextInput').placeholder = "タワマンの入り口は裏手です、等";
    } else { document.getElementById('memoTextInput').placeholder = "📍 GPSを取得中... メモを先に入力できます"; }
}
...
async function handleMemoImage(file, btnTextId, defaultText) { if (!file) return; if (selectedImageDataUrls.length >= 3) { alert("画像は最大3枚までです"); return; } document.getElementById(btnTextId).textContent = "⏳ 圧縮中"; const compressedData = await compressImage(file); selectedImageDataUrls.push(compressedData); document.getElementById('memoImagePreview').src = selectedImageDataUrls[selectedImageDataUrls.length - 1]; document.getElementById('memoImagePreviewContainer').style.display = 'block'; let countLabel = document.getElementById('memoImageCountLabel'); if(!countLabel) { countLabel = document.createElement('div'); countLabel.id = 'memoImageCountLabel'; countLabel.style.textAlign = 'center'; countLabel.style.fontSize = '12px'; countLabel.style.color = '#1A73E8'; countLabel.style.fontWeight = 'bold'; countLabel.style.marginTop = '4px'; document.getElementById('memoImagePreviewContainer').appendChild(countLabel); } countLabel.textContent = `${selectedImageDataUrls.length}枚選択中 (最大3枚)`; if (selectedImageDataUrls.length >= 3) { document.getElementById('memoCameraBtnText').parentNode.style.display = 'none'; document.getElementById('memoImageBtnText').parentNode.style.display = 'none'; } document.getElementById(btnTextId).textContent = defaultText; }
...
async function loadMemosToMap() { const q = query(collection(db, "memos"), orderBy("createdAt", "desc")); const snap = await getDocs(q); allMemosData = []; latestMemos = []; let count = 0; snap.forEach(docSnap => { const data = { id: docSnap.id, ...docSnap.data() }; allMemosData.push(data); if (count < 3) { latestMemos.push(data); count++; } }); applyFilters(); }
...
function showCurrentGroupMemo() { if (currentGroupMemos.length === 0) return; const targetMemo = currentGroupMemos[currentGroupIndex]; openMemoBottomSheet(targetMemo); const navBar = document.getElementById('memoGroupNav'); if (navBar) { if (currentGroupMemos.length > 1) { navBar.style.display = 'flex'; document.getElementById('textGroupCount').textContent = `付近に${currentGroupMemos.length}件のメモ (${currentGroupIndex + 1}/${currentGroupMemos.length})`; } else { navBar.style.display = 'none'; } } }
...
function openMemoBottomSheet(props) {
    ...
    let textContent = ""; if (fullData.buildingName) textContent += `🏢 ${fullData.buildingName}\n`; if (fullData.address) textContent += `📍 ${fullData.address}\n\n`; textContent += fullData.text || "";
    document.getElementById('sheetText').textContent = textContent.trim() || "メモがありません";
    const diffMin = Math.floor((Date.now() - fullData.createdAt) / 60000); let timeStr = "たった今"; if (diffMin >= 60 * 24) timeStr = `${Math.floor(diffMin / (60 * 24))}日前`; else if (diffMin >= 60) timeStr = `${Math.floor(diffMin / 60)}時間前`; else if (diffMin > 0) timeStr = `${diffMin}分前`;
    document.getElementById('sheetTime').textContent = timeStr; document.getElementById('sheetAuthorName').textContent = fullData.senderName; document.getElementById('sheetLikeCount').textContent = fullData.likesCount || 0; if (document.getElementById('sheetReportCount')) document.getElementById('sheetReportCount').textContent = fullData.reportCount || 0;
    ...
    const btnNavHere = document.getElementById('btnNavHere'); if (btnNavHere) { btnNavHere.onclick = () => { window.open(`https://www.google.com/maps/dir/?api=1&destination=${fullData.lat},${fullData.lng}`, '_blank'); }; }
    ...
    if(images.length > 0) { imgContainer.style.display = 'block'; bottomPadding = 520; scrollArea.innerHTML = ''; dotsArea.innerHTML = ''; images.forEach((url, idx) => { const img = document.createElement('img'); img.src = url; img.onclick = () => openImageViewer(url); scrollArea.appendChild(img); const dot = document.createElement('div'); dot.className = 'dot' + (idx === 0 ? ' active' : ''); dotsArea.appendChild(dot); }); if (images.length > 1) { counterLabel.style.display = 'block'; counterLabel.textContent = `1 / ${images.length}`; dotsArea.style.display = 'flex'; scrollArea.onscroll = () => { const index = Math.round(scrollArea.scrollLeft / scrollArea.clientWidth); counterLabel.textContent = `${index + 1} / ${images.length}`; const dots = dotsArea.querySelectorAll('.dot'); dots.forEach((d, i) => { if (i === index) d.classList.add('active'); else d.classList.remove('active'); }); }; } else { counterLabel.style.display = 'none'; dotsArea.style.display = 'none'; } } else { imgContainer.style.display = 'none'; }
    map.easeTo({ center: [fullData.lng, fullData.lat], padding: { top: 90, bottom: bottomPadding, left: 0, right: 0 }, duration: 350 });
    document.getElementById('memoActionPanel').classList.remove('show'); document.getElementById('filterBottomSheet').classList.remove('show'); const sheet = document.getElementById('memoBottomSheet'); sheet.classList.remove('peek'); sheet.classList.add('show');
}
...
document.getElementById('btnEditMemo')?.addEventListener('click', () => {
    ...
    document.getElementById('memoBottomSheet').classList.remove('show'); document.getElementById('memoBottomSheet').classList.remove('peek'); const panel = document.getElementById('memoActionPanel'); panel.classList.remove('peek'); panel.style.transform = ''; panel.classList.add('show'); 
    if (tempPinMarker) { tempPinMarker.remove(); tempPinMarker = null; } 
    const wrapper = document.createElement('div'); wrapper.className = 'marker-wrapper'; const pin = document.createElement('div'); pin.className = 'sharp-temp-pin pop-animation'; wrapper.appendChild(pin); tempPinMarker = new maplibregl.Marker({ element: wrapper, anchor: 'bottom' }).setLngLat(targetLngLat).addTo(map);
...
document.getElementById('closeModalBtn').addEventListener('click', () => { document.getElementById('profileModalOverlay').style.display = 'none'; document.getElementById('profileModal').style.display = 'none'; }); document.getElementById('profileModalOverlay').addEventListener('click', () => { document.getElementById('profileModalOverlay').style.display = 'none'; document.getElementById('profileModal').style.display = 'none'; }); window.openImageViewer = (url) => { document.getElementById('fullSizeImage').src = url; document.getElementById('imageViewerOverlay').style.display = 'block'; document.getElementById('imageViewerModal').style.display = 'block'; }; const closeViewer = () => { document.getElementById('imageViewerOverlay').style.display = 'none'; document.getElementById('imageViewerModal').style.display = 'none'; }; document.getElementById('closeImageViewerBtn').addEventListener('click', closeViewer); document.getElementById('imageViewerOverlay').addEventListener('click', closeViewer);
const btnGroupPrev = document.getElementById('btnGroupPrev'); const btnGroupNext = document.getElementById('btnGroupNext'); if (btnGroupPrev) { btnGroupPrev.addEventListener('click', () => { if (currentGroupIndex > 0) { currentGroupIndex--; showCurrentGroupMemo(); } }); } if (btnGroupNext) { btnGroupNext.addEventListener('click', () => { if (currentGroupIndex < currentGroupMemos.length - 1) { currentGroupIndex++; showCurrentGroupMemo(); } }); }
const memoSheet = document.getElementById('memoBottomSheet'); let touchStartY = 0; memoSheet.addEventListener('touchstart', (e) => { touchStartY = e.touches[0].clientY; }, { passive: true }); memoSheet.addEventListener('touchend', (e) => { const diff = e.changedTouches[0].clientY - touchStartY; if (diff > 50 && memoSheet.classList.contains('show') && !memoSheet.classList.contains('peek')) { if (memoSheet.scrollTop === 0) memoSheet.classList.add('peek'); } else if (diff < -50 && memoSheet.classList.contains('peek')) { memoSheet.classList.remove('peek'); } }); memoSheet.addEventListener('click', (e) => { if (e.target.id !== 'btnCloseMemoSheet' && !e.target.classList.contains('btn-group-nav') && memoSheet.classList.contains('peek')) { memoSheet.classList.remove('peek'); } }); const btnCloseSheet = document.getElementById('btnCloseMemoSheet'); if (btnCloseSheet) { btnCloseSheet.addEventListener('click', (e) => { e.stopPropagation(); memoSheet.classList.remove('show'); memoSheet.classList.remove('peek'); const navBar = document.getElementById('memoGroupNav'); if (navBar) navBar.style.display = 'none'; currentOpenMemoId = null; applyFilters(); }); }
...
if (btnSelectWeatherArea) {
    btnSelectWeatherArea.addEventListener('click', () => {
        ...
        weatherTargetMark.style.display = 'block';
        btnWeatherConfirm.style.display = 'flex';
        targetMode = 'weather';
    });
}
