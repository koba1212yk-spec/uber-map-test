...
let myProfileName = "名無し配達員"; // 初期値
let selectedImageDataUrl = null;
let bbsSelectedImageDataUrl = null;
let lastBbsDoc = null; 
let currentLocationMarker = null;
let currentOpenMemoId = null; // ボトムシートで開いているメモのID
...
// ==========================================
// 🗺️ MapTiler (Google Maps風 ライトモード)
// ==========================================
...
const map = new maplibregl.Map({
    ...
    zoom: 14,
    attributionControl: false // プロフィールに記載するため非表示
});

map.on('load', () => {
    // 🟢 メモ用 GeoJSONソース (クラスタリング有効)
    map.addSource('memos', {
        type: 'geojson',
        data: { type: 'FeatureCollection', features: [] },
        cluster: true,
        clusterMaxZoom: 14, // これ以上拡大するとバラける
        clusterRadius: 50
    });
    ...
    map.on('click', 'unclustered-point', (e) => {
        ...
        openMemoBottomSheet(props);
    });
    map.on('click', 'unclustered-emoji', (e) => {
        const props = e.features[0].properties;
        openMemoBottomSheet(props);
    });
...
document.getElementById('btnSearchAddress').addEventListener('click', execSearch);
...
async function loadProfile() {
    try {
        ...
        if (snap.exists()) {
            ...
            document.querySelectorAll('input[name="vehicleTag"]').forEach(cb => cb.checked = pData.vehicles?.includes(cb.value)); 
            document.querySelectorAll('input[name="serviceTag"]').forEach(cb => cb.checked = pData.services?.includes(cb.value));
        }
    } catch (e) {}
}
...
document.getElementById('saveProfileBtn').addEventListener('click', async () => {
    ...
    try { 
        ...
        await loadProfile(); 
        document.getElementById('profileEditMode').style.display = 'none'; document.getElementById('profileReadMode').style.display = 'block'; 
    } catch (error) {}
});
...
document.getElementById('addMemoModeBtn').addEventListener('click', () => { document.getElementById('mapCenterTarget').style.display = 'block'; document.getElementById('mapControlsContainer').style.display = 'none'; document.getElementById('memoActionPanel').classList.add('show'); document.getElementById('memoPhaseLoc').style.display = 'block'; document.getElementById('memoPhaseForm').style.display = 'none'; });
...
const closeMemoForm = () => { document.getElementById('mapCenterTarget').style.display = 'none'; document.getElementById('memoActionPanel').classList.remove('show'); document.getElementById('mapControlsContainer').style.display = 'flex'; document.getElementById('btnRemoveImage').click(); document.getElementById('memoTextInput').value = ""; };
document.getElementById('btnCancelMemoLoc').addEventListener('click', closeMemoForm); document.getElementById('btnCancelForm').addEventListener('click', closeMemoForm);
...
document.getElementById('btnSaveMemo').addEventListener('click', async () => {
    ...
    const center = map.getCenter(); const category = document.querySelector('input[name="memoCat"]:checked').value;
    const isShowName = document.getElementById('chkShowName').checked;
    const finalSenderName = isShowName ? myProfileName : "匿名ドライバー"; // トグルで判定
    ...
    try {
        ...
        await addDoc(collection(db, "memos"), { lat: center.lat, lng: center.lng, category: category, text: text, imageUrl: finalUrl, senderId: currentUserId, senderName: finalSenderName, likesCount: 0, createdAt: Date.now() }); // GeoJSON用にTimestampではなく数値保存
...
async function loadMemosToMap() {
    ...
    snap.forEach((docSnap) => {
        ...
        features.push({
            ...
            properties: { id: docSnap.id, emoji: emoji, category: data.category, text: data.text || "", imageUrl: data.imageUrl || "", senderId: data.senderId, senderName: data.senderName || "匿名ドライバー", likesCount: data.likesCount || 0, createdAt: data.createdAt || 0 }
        });
    });
...
function openMemoBottomSheet(props) {
    ...
    // プロフィール閲覧
    document.getElementById('sheetAuthorContainer').onclick = () => {
        if (props.senderName !== "匿名ドライバー") openProfileModal(props.senderId, props.senderName);
    };
...
document.getElementById('btnLikeMemo').addEventListener('click', async () => {
    ...
    try {
        ...
        if(memoSnap.exists()) {
            const targetId = memoSnap.data().senderId;
            await setDoc(doc(db, "profiles", targetId), { totalLikes: increment(1) }, { merge: true });
        }
        document.getElementById('sheetLikeCount').textContent = parseInt(document.getElementById('sheetLikeCount').textContent) + 1;
        loadMemosToMap(); // 裏のデータも更新
    } catch(e) {}
});
...
async function openProfileModal(targetId, targetName) {
    ...
    document.getElementById('modalArea').textContent = "読込中..."; document.getElementById('modalTime').textContent = "読込中..."; document.getElementById('modalLikesCount').textContent = "-";
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            document.getElementById('modalArea').textContent = data.mainArea || "未設定"; document.getElementById('modalTime').textContent = data.mainTime || "未設定";
            document.getElementById('modalLikesCount').textContent = data.totalLikes || 0;
...
async function loadBbsTimeline(isMore = false) {
    ...
    snapshot.forEach((docSnap) => {
        ...
        card.innerHTML = `
            <div class="bbs-avatar-column"><div class="bbs-avatar" style="background-color: ${avatarColors[aId]};">${avatarEmojis[aId]}</div></div>
            <div class="bbs-card-content">
                ...
                ${data.imageUrl ? `<img src="${data.imageUrl}" class="bbs-card-image" onclick="openImageViewer('${data.imageUrl}')">` : ""}
